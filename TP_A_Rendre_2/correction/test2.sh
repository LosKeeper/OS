...
echo -n "Test 1.4 - Débordement de tableau.................................."
raz
NOM="PROD1234567"               # 11 octets
./vendeur $NOM 1               2> $TMP.verr && fail "vendeur : nom un peu long"
touch $NOM
./client $NOM 1                2> $TMP.verr && fail "client : nom un peu long"

NOM="PROD1unnomtrestreslong"
NOM="$NOM$NOM"
NOM="$NOM$NOM"
NOM="$NOM$NOM"
./vendeur $NOM 1               2> $TMP.verr && fail "vendeur : nom très long"
touch $NOM
./client $NOM 1                2> $TMP.verr && fail "client : nom très long"
...
echo -n "Test 2.4 - Fonctionnalité basique avec reappro d'un seul vendeur..."
raz
./vendeur PROD1 1 2> $TMP.verr1        || fail "erreur vendeur 1 PROD1 2"
[ -f PROD1 ]                           || fail "PROD1 pas créé"
./client PROD1 5 > $TMP.cout1 2> $TMP.cerr1 &
LPID=$!
./client PROD1 5 > $TMP.cout2 2> $TMP.cerr2 &
LPID="$LPID $!"
# stock = <0>, attente = <c1, c2>
for i in $(seq 1 8)
do
    ./vendeur PROD1 1 2>> $TMP.verr
done
n=$(ps_teste_liste $LPID)
[ $n != 1 ]                           && fail "1 client devrait attendre ($n)"
# stock = <0>, attente = <-, c2> (ou bien <c1, ->)
for i in $(seq 1 2)
do
    ./vendeur PROD1 1 2>> $TMP.verr
done
n=$(ps_teste_liste $LPID)
[ $n != 0 ]                           && fail "aucun client ne devrait attendre ($n)"
./vendeur PROD1 0 2>> $TMP.verr
wait $LPID
...
echo -n "Test 4.3 - Sections critiques arrêtées trop tôt...................."
raz
N=100
./vendeur PROD1 1 2> $TMP.verr0      || fail "erreur vendeur 0"
[ -f PROD1 ]                         || fail "PROD1 pas créé"
# on lance $N fois le motif : j'achète le dernier produit dispo
for i in $(seq 1 $N)
do
    ./client PROD1 1 > $TMP.cout$i 2> $TMP.cerr$i-1 &
    LPID="$!"
    ./client PROD1 1 > $TMP.cout$i 2> $TMP.cerr$i-2 &
    LPID="$LPID $!"
    msleep 50
    n=$(ps_teste_liste $LPID)
    [ $n = 1 ]                        || fail "tour $i : 1 client devrait attendre"
    ./vendeur PROD1 2 > $TMP.verr$i 2> $TMP.err$i
    msleep 50
    n=$(ps_teste_liste $LPID)
    [ $n = 0 ]                        || fail "tour $i : aucun client ne devrait attendre"
    wait
done
